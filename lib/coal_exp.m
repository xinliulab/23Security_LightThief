function [data] = coal_exp(idx,az,aco_cbsize,savedata,save_folder,BPU,PA_list)
    % PA: calibration vector obtained from controlled environment
    % PA2: calibration vector obtained from uncontrolled environment
    tic
    close all;
    % repeat = 10;
    data = [];
    ...
    data.az                  = az;
    data.rx_ex_probes        = zeros(BPU.PN_INTERVAL, length(data.az), length(PA_list));
%     data.rx_ex_probes2       = zeros(BPU.PN_INTERVAL, length(data.az));
    data.rx_aco_probes       = zeros(BPU.PN_INTERVAL, 124);
    data.rx_cs_probes        = zeros(BPU.PN_INTERVAL, 124, length(PA_list));
    ...
    data.snr_cs_dominantpath = zeros(length(aco_cbsize), length(PA_list));
...
%     debug_path_ang           = zeros(2, length(aco_cbsize));
    ...
    % Exhaustive search
    % test many calibration vectors
    for ii=1:length(PA_list)
        PA = PA_list(ii);
...
%     create_csi_codebook(124, 1, PA.ACTIVE_ANT(2:32), PA);
    create_csi_codebook(124, PA.REFANT, setdiff(PA.ACTIVE_ANT, [PA.REFANT]), PA);
...
%     v2_multipath   = zeros(length(PA.ACTIVE_ANT),length(aco_cbsize));
    v_dominantpath = zeros(length(PA.ACTIVE_ANT),length(aco_cbsize), length(PA_list));
...
%             v2_multipath(:,ii) = v2_multipath_;
            v_dominantpath(:,ii,jj) = v_dominantpath_./v_dominantpath_(PA.REFANT).*exp(1j*PA.PHASE_CAL);
        end
        PA = PA_list(1);
        % calculate ACO beams
        aco_activated_ant = [1:1+n/4];
        v_aco(setdiff(PA.ACTIVE_ANT, [PA.REFANT]),ii) = exp(1j*[-rel_phase_avg]); 
...
%     v2 = v_multipath./v_multipath(1,:).*exp(1j*PA2.PHASE_CAL);
    v3 = reshape(v_dominantpath, length(PA.ACTIVE_ANT), []);
%     v4 = v2_multipath./v2_multipath(1,:).*exp(1j*PA2.PHASE_CAL);
    v5 = v_aco;
...
%     data.snr_cs_multipath    = SNR2(0*length(aco_cbsize)+[1:length(aco_cbsize)]);
    data.snr_cs_dominantpath = SNR2(0*length(aco_cbsize)*length(PA_list)+[1:length(aco_cbsize)*length(PA_list)]);
%     data.snr_cs_multipath_v2 = SNR2(2*length(aco_cbsize)+[1:length(aco_cbsize)]);
    data.snr_aco             = SNR2(1*length(aco_cbsize)*length(PA_list)+[1:length(aco_cbsize)]);
    data.snr_11ad            = SNR2(1*length(aco_cbsize)*length(PA_list)+1*length(aco_cbsize)+[1:length(aco_cbsize)]);
    
    fprintf("CS SNR (different cal vector): %.2f\n", data.snr_cs_dominantpath);
    fprintf("ACO SNR: %.2f\n", data.snr_aco);
    fprintf("11ad SNR: %.2f\n", data.snr_11ad);
%     sv2beam(sv,[-90:1:90],[-50:1:50],PA_list(1),["CS 1", "CS 2", "CS 3", "ACO", "11ad"]);
