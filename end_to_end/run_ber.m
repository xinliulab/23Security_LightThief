...
n_trials = 6;
packet_ids = cell(1, 40);
for k = 1:numel(packet_ids)
    packet_ids{k} = sprintf('LT-%04d', k);
end
fig_dir = fullfile(fileparts(mfilename('fullpath')), 'figures');
if ~exist(fig_dir, 'dir'), mkdir(fig_dir); end

[frame_bits, truth_packets] = build_packet_stream(packet_ids, p.preamble);
...
for ii = 1:numel(eb_no)
    ...
    for trial = 1:n_trials
        ...
        res = decode_packet_stream(rec, p, packet_lengths, numel(packet_ids));
