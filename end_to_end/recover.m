...
%   Pipeline (paper Sec. 5.3): integrate-and-dump matched filter -> coarse CFO
%   removal -> per-bit timing recovery -> static phase de-rotation -> Costas
%   carrier tracking -> hard BPSK slice.  Operates at Nb samples per bit and
%   returns one recovered bit per OWC bit period (Manchester is already
%   demodulated by the harmonic selection).
